...
tools: Bash (gh issue/gh api only), Read, Grep, Glob
model: sonnet
---
...
**Important:** Bash tool use is restricted to `gh issue` and `gh api graphql` commands only - no arbitrary shell operations. This enforces the read-only boundary and prevents accidental or injected commands from modifying files or system state.
...
Do this:
1. `gh issue list --repo magnusvmt/vidcast --state open --limit 1000 --json number,title,body,url` to see everything open (epics and regular issues alike).
